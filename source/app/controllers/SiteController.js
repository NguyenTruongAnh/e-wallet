...
class SiteController {
    ...
    login(req, res) {
        ...
        res.render('login.hbs', { layout: 'emptyLayout', title: 'AVAT - Đăng nhập', flash: flash[0] })
    }
    ...
    register(req, res) {
        const flash = req.flash('flash') || ''
        res.render('register.hbs', { layout: 'emptyLayout', title: 'AVAT - Đăng ký', flash: flash[0] })
    }
    ...
    async Register(req, res) {
        ...
        const mailOptions = {
            ...
            to: email,
            subject: 'Đăng kí tài khoản ví điện tử AVAT',
            html: `
                <p>Chúc mừng bạn đã tạo tài khoản thành công. Đây là tài khoản của bạn.</p>
                <p><b>Tên tài khoản: </b>${username}</p>
                <p><b>Mật khẩu: </b>${password}</p>
                <p>Tài khoản của bạn đang chờ kích hoạt để sử dụng ví điện tử AVAT.</p>
            `
        }
    ...
    resetPassword(req, res) {
        const flash = req.flash('flash') || ''
        res.render('reset-password.hbs', { layout: 'emptyLayout', title: 'AVAT - Đổi mật khẩu', flash: flash[0] })
    }
    ...
    async ResetPassword(req, res) {
        ...
        req.session.account.password = password
        Account.findByIdAndUpdate({ _id: req.session.account._id }, 
            { password, isChangeDefaultPassword: true }, { new: true })
                .then(() => {
                    req.flash('flash', {
                        type: 'success',
                        intro: 'Thành công!',
    ...
    recovery(req, res) {
        const flash = req.flash('flash') || ''
        res.render('recovery.hbs', { layout: 'emptyLayout', title: 'AVAT - Khôi phục mật khẩu', flash: flash[0] })
    }
    ...
    recovery2(req, res) {
        if (req.session.email) {
            const flash = req.flash('flash') || ''
            res.render('recovery2.hbs', { layout: 'emptyLayout', title: 'AVAT - Khôi phục mật khẩu', flash: flash[0] })
        } else {
